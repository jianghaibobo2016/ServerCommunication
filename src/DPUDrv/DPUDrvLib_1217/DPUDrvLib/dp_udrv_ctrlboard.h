...
//typedef boost::function<int(DP_U8 *, DP_U32)> CtrlBoardRcvCallback;
...
DP_S32 DP_UDRV_CtrlBoard_SendIRLearnBuf(DP_U8 port, DP_U8 *irData,
		DP_U32 irDataLen);

DP_S32 DP_UDRV_CtrlBoard_SetIOValue(DP_U8 ioIdx, DP_U8 ioValue);

DP_S32 DP_UDRV_CtrlBoard_RS232Open(DP_U8 port,
		DP_UDRV_CTRL_BOARD_UartAttr_S attr);
DP_S32 DP_UDRV_CtrlBoard_RS232Close(DP_U8 port);
DP_S32 DP_UDRV_CtrlBoard_RS232SendData(DP_U8 port, DP_U8 *sendBuf,
		DP_U32 sendBufLen);
//do not block in callback,do not call the API here which will cause dead lock
DP_S32 DP_UDRV_CtrlBoard_RegisterRS232CallBack(DP_U8 port,
		CtrlBoardRcvCallback callback);

DP_S32 DP_UDRV_CtrlBoard_RS485Open(DP_U8 port,
		DP_UDRV_CTRL_BOARD_UartAttr_S attr);
DP_S32 DP_UDRV_CtrlBoard_RS485Close(DP_U8 port);
DP_S32 DP_UDRV_CtrlBoard_RS485SendData(DP_U8 port, DP_U8 *sendBuf,
		DP_U32 sendBufLen);
//do not block in callback,do not call the API here which will cause dead lock
DP_S32 DP_UDRV_CtrlBoard_RegisterRS485CallBack(DP_U8 port,
		CtrlBoardRcvCallback callback);
