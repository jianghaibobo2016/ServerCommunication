...
#ifndef UTIL_CRC16_H_
#define UTIL_CRC16_H_

const UINT16 CRCtbl[256] = { 0x0000, 0xC0C1, 0xC181, 0x0140, 0xC301, 0x03C0,
...
public:
	...
	/*static uint16_t CalcCrc(uint16_t nCrc,uint8_t cCrcData)
	 {
	 return ( nCrc >> 8 ) ^ CRCtbl[ ( (UINT8)nCrc ) ^ cCrcData ];
	 }*/

	static uint16_t CalcCrcAll(const void * pData, uint16_t len) {
		uint16_t ci, nCrc;
		nCrc = 0xffff;
		uint8_t* cData = (uint8_t*) pData;
		for (ci = 0; ci < len; ci++)
			nCrc = (nCrc >> 8) ^ CRCtbl[((unsigned char) nCrc) ^ cData[ci]];
		return nCrc;
	}
