...
FileUtil::AppendFile::AppendFile(StringArg filename) :
		_filename(filename.c_str()), fp_(::fopen(filename.c_str(), "ae")), // 'e' for O_CLOEXEC
		writtenBytes_(0) {
	...
	writtenBytes_ = statbuff.st_size;
...
int FileUtil::ReadSmallFile::readToString(int maxSize, String* content,
	...
	// BOOST_STATIC_ASSERT(sizeof(off_t) == 8); ///arm compile is not invalid
	assert(content != NULL);
	int err = err_;
	if (fd_ >= 0) {
		content->clear();
		...
		while (content->size() < implicit_cast<size_t>(maxSize)) {
			size_t toRead = std::min(
					implicit_cast<size_t>(maxSize) - content->size(),
					sizeof(buf_));
