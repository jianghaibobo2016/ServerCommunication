...
void muduo::net::defaultMessageCallback(const TcpConnectionPtr&, Buffer* buf,
		Timestamp) {
	buf->retrieveAll();
}
...
TcpConnection::TcpConnection(EventLoop* loop, const string& nameArg, int sockfd,
		...
		loop_(CHECK_NOTNULL(loop)), name_(nameArg), state_(kConnecting), reading_(
				true),
		//socket --> fd = accept
		socket_(new Socket(sockfd)), channel_(new Channel(loop, sockfd)), localAddr_(
				localAddr), peerAddr_(peerAddr), highWaterMark_(
				64 * 1024 * 1024) {
	...
	LOG_DEBUG << "TcpConnection::ctor[" << name_ << "] at " << this << " fd="
			<< sockfd;
...
//	int nZero = 0;
//	setsockopt(channel_->fd(), SOL_SOCKET, SO_SNDBUF, (char *) &nZero,
//			sizeof(nZero));
	...
	if (!channel_->isWriting() && outputBuffer_.readableBytes() == 0) {
		...
		if (nwrote >= 0) {
//			PrintBuff::printBufferByHex("write: ", data, len);
			LOG_INFO
					<< "==================================================================write: "
					<< nwrote;
			...
			if (remaining == 0 && writeCompleteCallback_) {
				...
				loop_->queueInLoop(
						boost::bind(writeCompleteCallback_,
								shared_from_this()));
			}
		} else // nwrote < 0
		{
...
void TcpConnection::forceClose() {
	...
	if (state_ == kConnected || state_ == kDisconnecting) {
		setState(kDisconnecting);
		loop_->queueInLoop(
				boost::bind(&TcpConnection::forceCloseInLoop,
						shared_from_this()));
	}
}
